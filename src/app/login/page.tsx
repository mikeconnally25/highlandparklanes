...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    router.push(data.user?.role === "staff" ? "/staff" : "/app");
    router.refresh();
  }
  ...
  return (
    <main className="lane-bg min-h-screen flex flex-col items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <h1 className="mt-8 text-2xl font-semibold text-cream">Welcome back</h1>
        <p className="mt-2 text-muted">Sign in to see this week&apos;s coupon.</p>
        ...
        <p className="mt-6 text-sm text-muted">
          New here?{" "}
          <Link href="/signup" className="text-amber-soft hover:underline">
            Create an account
          </Link>
        </p>
      </div>
